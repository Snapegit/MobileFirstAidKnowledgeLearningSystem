	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
					...
					<div class="search_view">
						...
						<div class="search_box">
							<el-select
								class="search_sel"
								clearable
								v-model="searchQuery.kechengfenlei" 
								placeholder="课程分类"
								>
								<el-option v-for="item in kechengfenleiLists" :label="item" :value="item"></el-option>
							</el-select>
						</div>
					</div>
					<div class="search_view">
						...
						<div class="search_box">
							<el-date-picker
								class="search_date"
								clearable
								v-model="searchQuery.faburiqiTimes"
								type="daterange"
								format="YYYY-MM-DD"
								value-format="YYYY-MM-DD"
								range-separator="至"
								start-placeholder="发布日期开始时间"
								end-placeholder="发布日期结束时间"
								/>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('jijiukecheng','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('jijiukecheng','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('jijiukecheng','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('jijiukecheng','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="视频" :resizable='true' :sortable='true' align="left" header-align="center">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="center">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('jijiukecheng','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('jijiukecheng','查看评论')" type="warning" @click="commentClick(scope.row.id)">查看评论</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const getList = () => {
		...
		if(searchQuery.value.kechengfenlei&&searchQuery.value.kechengfenlei!=''){
			params['kechengfenlei'] = searchQuery.value.kechengfenlei
		}
		if(searchQuery.value.faburiqiTimes&&searchQuery.value.faburiqiTimes.length){
			params['faburiqistart'] = searchQuery.value.faburiqiTimes[0]
			params['faburiqiend'] = searchQuery.value.faburiqiTimes[0]
		}
		context?.$http({
			url: `${tableName}/page`,
			method: 'get',
	...
	//搜索
	const kechengfenleiLists = ref([])
	const getkechengfenleiLists = () => {
		context?.$http({
			url: 'option/kechengfenlei/kechengfenlei',
			method:'get'
		}).then(res=>{
			kechengfenleiLists.value = res.data.data
		})
	}
	getkechengfenleiLists()
	const searchClick = () => {
	...
	const commentClick=(id)=>{
		context?.$router.push('/discussjijiukecheng?refid=' + id)
	}
	//初始化
	const init = () => {
		getList()
